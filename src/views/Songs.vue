      ...
      <table cellspacing="0" cellpadding="0">
        <tr ref="header">
          ...
          <th id="th-title" @click="sortBy('title')" :class="{ active: isSortActive('title') }">
            Title
            <IconCaretUp v-if="isSortActive('title')" :class="{ 'flip-vertical': isSortDesc() }" />
          </th>
          <th id="th-artist">Artist</th>
          <th id="th-album">Album</th>
          <th id="th-duration" @click="sortBy('duration')" :class="{ active: isSortActive('duration') }">
            Duration
            <IconCaretUp v-if="isSortActive('duration')" :class="{ 'flip-vertical': isSortDesc() }" />
          </th>
        </tr>
        <tr class="song" v-for="(song, index) in filteredSongs" :key="song.id" @click="selectSong(song)" @mouseover="hoveredSongIndex = index" @mouseout="hoveredSongIndex = -1" :class="{ active: isSongActive(song) }">
          ...
          <td id="td-duration">
            ...
            <IconHeart :class="{ active: isSongFavorite(song) || isSongHovered(index) }" @click="toggleFavoriteStatus(song)" />
          </td>
        </tr>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    toggleFavoriteStatus(song) {
      this.auth.toggleFavorite(song.id);
    },
  },
  mounted() {
